...
class SentenceClassifier(nn.Module):
    ...
    def __init__(self, n_classes=2, drop_rate=0.3, xavier_init=False, hidden_size=None, use_amp=True):
        ...
        '''
        self.bert = BertModel.from_pretrained('./Model/', hidden_dropout_prob=drop_rate)
        ...
        if hidden_size is None:  # No hidden layer
            ...
            if xavier_init:
                nn.init.xavier_uniform_(self.embed_to_logits.weight, gain=1.0)  # Xavier initialization
        else:  # Single hidden layer
            self.hidden_layer = True
            self.drop_embed = nn.Dropout(p=drop_rate)
            self.embed_to_hidden = nn.Linear(self.bert.config.hidden_size, hidden_size)
            self.drop_hidden = nn.Dropout(p=drop_rate)
            self.hidden_to_logits = nn.Linear(hidden_size, n_classes)
            if xavier_init:
                nn.init.xavier_uniform_(self.embed_to_logits.weight, gain=1.0)  # Xavier initialization
                nn.init.xavier_uniform_(self.hidden_to_logits.weight, gain=1.0)  # Xavier initialization
...
def train_epoch(model, data_loader, loss_fn, optimizer, device, num_instances, scheduler, logger, scaler,
    ...
    for d in data_loader:  # For one batch of data
        ...
        loss = loss.mean()  # Multi-GPU training
...
if __name__ == '__main__':
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = "2"  # Only use the third GPU for calculation
    ...
    use_bfsc = True  # Whether the "BertForSequenceClassification" model is used instead of the plain BERT model
    ...
    weight_decay_rates = [0.01]
    drop_rates = [0.01]  # Same drop rate of all dropout layers
    ...
    logger.info(f"Drop rates (same for all dropout layers): {drop_rates}")
    ...
    val_f1_grid = np.zeros((len(train_batch_sizes), len(learning_rates), len(weight_decay_rates), len(drop_rates)))
    epoch_grid = np.zeros((len(train_batch_sizes), len(learning_rates), len(weight_decay_rates), len(drop_rates)))
    ...
    for i, bs in enumerate(train_batch_sizes):
        for j, lr in enumerate(learning_rates):
            for k, wd in enumerate(weight_decay_rates):
                for p, dr in enumerate(drop_rates):
                    ...
                    if use_bfsc:
                        try:
                            model = BertForSequenceClassification.from_pretrained('./Model_bfsc/',
                        ...
                        model = SentenceClassifier(n_classes=2, drop_rate=dr, xavier_init=xavier_init,
                    ...
                    logger.info('-' * 100)
                    logger.info(f'Batch size: {bs}, initial learning rate: {lr}, weight decay rates: {wd}, '
                                f'drop rate: {dr}, total steps: {total_steps}')
                    logger.info('-' * 100)
                            ...
                            if val_f1 > best_val_f1_overall:
                                if gpu_count > 1:
                                    torch.save(model.module.state_dict(),
                                               'Model/best_model_state_' + str(dataset) + '.pt')
                                else:
                    ...
                    val_f1_grid[i, j, k, p] = best_val_f1
                    epoch_grid[i, j, k, p] = best_epoch
                    t = time.time() - t0
                    t_tot += t
    ...
    logger.info(
        f'Best parameters: batch size = {best_bs}, learning rate = {best_lr}, weight decay rate = {best_wd}, '
        f'best drop rate = {best_dr}, training epoch = {best_epoch_overall}')

    # Load best model
    logger.info('')
    logger.info('Loading best model: ')
    if use_bfsc:  # TODO: correct error
        model = BertForSequenceClassification.from_pretrained('./Model_bfsc/')
        model.load_state_dict(
            torch.load('Model/best_model_state_' + str(dataset) + '.pt', map_location=device))
    else:
